...
const getRoleIcon = (role: string) => {
  switch (role) {
    case "student":
      ...
      return (
        <svg viewBox="0 0 24 24" className="w-7 fill-white">
    ...
    default:
      // Default person icon
      return (
        <svg viewBox="0 0 15 15" className="w-7 fill-white">
...
export function ProfileCard({ userName, role, department, email }: ProfileCardProps) {
  return (
    <div className="flex items-center gap-3">
      <div className={`flex justify-center items-center w-14 h-14 rounded-full shadow-md bg-gradient-to-r ${getGradientColors(role)}`}>
        {getRoleIcon(role)}
      </div>
      <div>
